...
from torchao.float8.hifloat8_utils import is_hifloat8_dtype, to_hifloat8
...
def amax_to_scale(
    ...
    if is_hifloat8_dtype(float8_dtype):
        # HiFloat8 uses an internal cast kernel; treat scale as 1.0 so
...
def to_fp8_saturated(x: torch.Tensor, float8_dtype: torch.dtype):
    ...
    """
    if is_hifloat8_dtype(float8_dtype):
        # HiFloat8 conversion handled by torch_npu cast kernels.
...
def fp8_tensor_statistics(
    ...
    """
    if is_hifloat8_dtype(float8_dtype):
        # Best-effort statistics for HiFloat8: count zeros and skip max count.
        tensor_orig_type = (
